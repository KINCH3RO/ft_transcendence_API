...
export class ChannelService {
	...
	async create(createChannelDto: CreateChannelDto, id: string) {
		if (!createChannelDto.imageUrl)
			createChannelDto.imageUrl = this.getRandomImage("/public/assets/chAvatars")
	...
	async findChannelByName(currentUserId: string, name: string) {
		...
		list.map((item) => {
			let channel_user: channelUser = item.channels.find(
