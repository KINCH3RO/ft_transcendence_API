import { Controller, HttpException, HttpStatus, Param, Post, Query, UploadedFiles, UseInterceptors } from '@nestjs/common';
...
export class UploadController {
	...
	uploadFile(@UploadedFiles() files: Express.Multer.File[], @Param("dir") uploadDir: string) {
		...
		for (let i = 0; i < files.length; i++) {
			let fileName = this.uploadService.genRandomFileName(files[i].originalname);
